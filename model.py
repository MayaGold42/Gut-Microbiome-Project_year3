...
df_fme['shanon_avg'] = df_fme.groupby('participant_id')['shannon_diversity'].transform('mean')
# Calculate sample shanon deviation
df_fme['shanon_deviation'] = df_fme['shannon_diversity'] - df_fme['shanon_avg']

#Regression: predict continuous distance from avg Shannon diversity
print("\n Regression: predict continuous distance from avg Shannon diversity")

#Use the same features, but keep Shannon diversity as a continuous target
df_reg_dev_clean = df_fme[feature_cols + ["shanon_deviation", "participant_id"]].dropna()

#Prepare regression data
X_dev_reg = df_reg_dev_clean[feature_cols]
y_dev_reg = df_reg_dev_clean["shanon_deviation"]
...
sns.histplot(df_reg_dev_clean["shanon_deviation"], bins=30, kde=True)
