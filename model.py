...
for name, model in models.items():
    ...
    if mean_auc > best_auc:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            best_model_feat_imp = np.abs(model.coef_[0])

print(f"\n Best Model: {best_model_name} (Mean AUC = {best_auc:.3f})")

#Feature importance for best model
if best_model_feat_imp is not None:
    ...
    if best_model_name == "Logistic Regression":
        plt.title('Feature Importance (Logistic Regression Absolute Coefficients)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(r'c:\Users\linoy\Documents\proj3\feature_importance.png', dpi=150)
    plt.show()
    print("\nPlot saved successfully to 'feature_importance.png'")